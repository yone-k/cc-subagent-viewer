# subagent-viewer

Claude Code のサブエージェントセッションをリアルタイムで監視するターミナル UI (TUI) ツール。

タスクの進捗状況、デバッグログ、ファイル変更履歴、セッション統計をひとつの画面で確認できます。

## インストール

Go 1.25 以上が必要です。

```bash
go install github.com/yone/subagent-viewer@latest
```

または、リポジトリをクローンしてビルド:

```bash
git clone https://github.com/yone/subagent-viewer.git
cd subagent-viewer
go build -o subagent-viewer .
```

## 使い方

### セッション選択モード

引数なしで起動すると、`~/.claude/history.jsonl` からセッション一覧を表示します。

```bash
subagent-viewer
```

各セッションにはプロジェクトパス、相対時刻、最初のユーザー入力、利用可能なデータの種類 (`[Tasks|Logs|Files]`) が表示されます。fuzzy フィルタリングで絞り込みが可能です。

### 直接指定モード

セッション UUID を引数に渡すと、セレクターをスキップして直接ビューアーを開きます。

```bash
subagent-viewer 7ba50137-65c8-4349-b420-cdce14c38d2a
```

## タブ

### Tasks

サブエージェントのタスク一覧をリアルタイム表示します。

- 進捗バーで完了状況を可視化
- ステータスアイコン: `✓` 完了 / `●` 進行中 / `○` 保留 / `✗` ブロック中
- `Enter` でタスク詳細の展開/折畳み
- タスクファイルの変更を fsnotify で即時検知

### Logs

デバッグログ (`~/.claude/debug/{UUID}.txt`) を 500ms ポーリングで追跡します。

- 7種類のログレベルをキー1つでフィルタトグル
- `/` でインライン検索（リアルタイム絞り込み）
- `f` で自動スクロール (AUTO/MANUAL) を切替
- 最大 10,000 エントリをリングバッファで保持

### Files

ファイル変更履歴 (`~/.claude/file-history/{UUID}/`) を監視します。

- 同一ファイルのバージョンをグループ化して表示
- `Enter` で展開/折畳み、`Esc` で全折畳み

### Stats

`~/.claude.json` からセッション統計を表示します。

- コスト、所要時間、入出力トークン数
- モデル別の使用量内訳
- 最新セッション以外を表示中の場合は警告を表示（`.claude.json` は最新セッションの統計のみ保持するため）

## キーバインド

### グローバル

| キー | 動作 |
|------|------|
| `1` `2` `3` `4` | タブ切替 (Tasks / Logs / Files / Stats) |
| `Tab` / `Shift+Tab` | 次 / 前のタブ |
| `q` / `Ctrl+C` | 終了 |

### Tasks / Files 共通

| キー | 動作 |
|------|------|
| `↑` / `k` | 上へ移動 |
| `↓` / `j` | 下へ移動 |
| `Enter` | 詳細展開 / 折畳み |

### Logs

| キー | 動作 |
|------|------|
| `D` `E` `W` `M` `S` `T` `A` | ログレベルフィルタ (DEBUG / ERROR / WARN / MCP / STARTUP / META / ATTACHMENT) |
| `/` | 検索モード |
| `Esc` | 検索終了 |
| `f` | AUTO / MANUAL スクロール切替 |
| `↑` `↓` / `k` `j` | スクロール |

### Files

| キー | 動作 |
|------|------|
| `Esc` | 全グループを折畳み |

## データソース

すべて `~/.claude/` 配下のファイルを読み取り専用で参照します。

| パス | 内容 |
|------|------|
| `history.jsonl` | セッション履歴 (JSONL) |
| `tasks/{UUID}/*.json` | タスク状態。`.lock` はセッションがアクティブ中であることを示す |
| `debug/{UUID}.txt` | タイムスタンプ付きデバッグログ |
| `file-history/{UUID}/` | ファイル変更のスナップショット (`{hash}@v{N}` 形式) |
| `~/.claude.json` | プロジェクト別統計 (最新セッションのみ) |

## 技術スタック

- [Bubble Tea](https://github.com/charmbracelet/bubbletea) — Elm アーキテクチャ TUI フレームワーク
- [Lip Gloss](https://github.com/charmbracelet/lipgloss) — ターミナルスタイリング
- [Bubbles](https://github.com/charmbracelet/bubbles) — TUI コンポーネント (list, textinput, key)
- [fsnotify](https://github.com/fsnotify/fsnotify) — クロスプラットフォームファイル監視

## ライセンス

MIT
