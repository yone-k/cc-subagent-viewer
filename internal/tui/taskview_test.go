...
func TestTaskView_SwitchToAgentsMode(t *testing.T) {
	m := NewTaskViewModel()
	m.width = 80
	m.height = 24
	m.agents = []claude.SubagentInfo{
		{AgentID: "a1", Slug: "agent-1", Prompt: "test", EntryCount: 3},
	}

	// Press 'a' to switch to agents mode
	newModel, _ := m.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune("a")})
	tv := newModel.(TaskViewModel)

	if tv.mode != TaskViewModeAgents {
		t.Errorf("mode = %d, want TaskViewModeAgents (%d)", tv.mode, TaskViewModeAgents)
	}
}

func TestTaskView_AgentsEscapeReturns(t *testing.T) {
	m := NewTaskViewModel()
	...
	if tv.mode != TaskViewModeTasks {
		t.Errorf("mode = %d, want TaskViewModeTasks (%d)", tv.mode, TaskViewModeTasks)
	}
}

func TestTaskView_ConversationView(t *testing.T) {
	m := NewTaskViewModel()
	m.mode = TaskViewModeConversation
	m.currentAgentID = "agent1"
	m.width = 80
	m.height = 24
	m.conversations["agent1"] = []claude.ConversationEntry{
		{
			Type:    claude.EntryTypeUser,
			Content: []claude.ContentBlock{{Type: "text", Text: "Hello agent"}},
		},
		{
			Type:    claude.EntryTypeAssistant,
			Content: []claude.ContentBlock{{Type: "text", Text: "Hello! I can help."}},
		},
	}
	m.conversationInfo["agent1"] = &claude.SubagentInfo{AgentID: "agent1", Slug: "test-agent"}

	view := m.View()

	if !strings.Contains(view, "[USER]") {
		t.Error("view should contain [USER] tag")
	}
	if !strings.Contains(view, "[ASSISTANT]") {
		t.Error("view should contain [ASSISTANT] tag")
	}
	if !strings.Contains(view, "Hello agent") {
		t.Error("view should contain user message text")
	}
	if !strings.Contains(view, "Hello! I can help.") {
		t.Error("view should contain assistant message text")
	}
}

func TestTaskView_SubagentsDiscoveredMsg(t *testing.T) {
	m := NewTaskViewModel()

	agents := []claude.SubagentInfo{
		{AgentID: "a1", Slug: "first-agent", Prompt: "Do something", EntryCount: 5},
		{AgentID: "a2", Slug: "second-agent", Prompt: "Do another thing", EntryCount: 3},
	}
	...
	if len(tv.agents) != 2 {
		t.Errorf("agents count = %d, want 2", len(tv.agents))
	}
}

func TestTaskView_ConversationEscapeReturns(t *testing.T) {
	m := NewTaskViewModel()
	m.mode = TaskViewModeConversation
	m.width = 80
	m.height = 24
...
func TestTaskView_AgentEnterOpensConversation(t *testing.T) {
	m := NewTaskViewModel()
	m.mode = TaskViewModeAgents
	m.width = 80
	m.height = 24
	m.agents = []claude.SubagentInfo{
		{AgentID: "a1", Slug: "test-agent", Prompt: "test", EntryCount: 2},
	}
	m.agentSelected = 0
	...
	if tv.mode != TaskViewModeConversation {
		t.Errorf("mode = %d, want TaskViewModeConversation (%d)", tv.mode, TaskViewModeConversation)
	}
	if tv.currentAgentID != "a1" {
		t.Errorf("currentAgentID = %q, want %q", tv.currentAgentID, "a1")
	}
}
